...
void PrintPreviewUI::PrintPdf(
  ...
  if (memcpy_s(buff.get(), data_size, base::as_string_view(*print_data).data(), data_size) != EOK) {
    LOG(ERROR) << "PrintPdf memcpy_s failed.";
    return;
  }
#else
  memcpy(buff.get(), base::as_string_view(*print_data).data(), data_size);
#endif
  ohos::adapter::print::PrintAdapter::GetInstance().SetPdfBuff(buff, data_size);
}
#endif
