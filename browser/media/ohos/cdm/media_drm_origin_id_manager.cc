...
bool CanPreProvision(bool is_per_application_provisioning_supported,
  ...
  if (!token_value) {
    LOG(WARNING) << __func__
                 << " [WiseplayDRM] previous provision token not exist, can "
                    "not skip provision.";
    return false;
  }
  ...
  if (!expiration_time) {
    ...
    LOG(WARNING) << __func__
                 << " [WiseplayDRM] invalid expiration time of token, can not "
                    "skip provision.";
    return false;
  }
  ...
  if (base::Time::Now() > *expiration_time) {
    RemoveExpirableToken(origin_id_dict);
    LOG(WARNING) << __func__
                 << " [WiseplayDRM] token expired, can not skip provision.";
    return false;
  }
...
base::UnguessableToken TakeFirstOriginId(PrefService* const pref_service) {
  ...
  if (!origin_ids || origin_ids->empty()) {
...
void MediaDrmOriginIdManager::ResumePreProvisionIfNecessary(
  ...
  StartProvisioningAsync(true); /*run_in_background=*/
}
...
void MediaDrmOriginIdManager::GetOriginId(ProvisionedOriginIdCB callback) {
  ...
  if (!is_provisioning_) {
    is_provisioning_ = true;
    StartProvisioningAsync(!origin_id.is_empty()); /*run_in_background=*/
  }
...
void MediaDrmOriginIdManager::OriginIdProvisioned(
  ...
  StartProvisioningAsync(
      pending_provisioned_origin_id_cbs_.empty()); /*run_in_background=*/
}
